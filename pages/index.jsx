    ...
    <div>
      <header className="">
        <div className="flex justify-between">
          <div className="bg-pink-200">search</div>
            <h1 className="text-2xl text-gray-300 font-bold">The Window</h1> 
          <div className="bg-pink-200">sub</div>
        </div>
      ...
      <main className="max-w-container mx-auto grid grid-cols-12 justify-evenly text-gray-200">
        {/* top box region */}
          {ticker.map(p=>( <ListCard post={p} type="ticker" />
          ))}
        {/* top box region over */}
        ...
        {leftFeatures.map(p=>( <Card size="m" post={p}/>
          ))}
        </aside>
        {/* center */}
        <section className="col-span-6"><Card size="xl" post={mainFeature[0]}/></section>
        {/* right side */}
        <aside className="col-span-3">
        {rightFeatures.map(p=>(<Card size="m" post={p}/>
          ))}
        </aside>
        {/* main feature layout region over */}

        {/* secondary layout grid region */}
        {secondaries.map(p=>(<article key={p._id} className="py-8 col-span-3">
        <Card size="m" post={p}/></ article>
          ))}
        ...
        <div className="col-span-4">
          ...
          <Text size="m">Here there will be bounties for wanted pieces of writing. Maybe paid out via stablecoin.</Text>
        ...
        <footer className="bg-gray-600">
          ...
          <div className="flex justify-evenly max-w-4xl mx-auto py-12">
            <section className="px-4">
              <Text size="m"><strong>About</strong></Text>
            ...
            <section className="px-4">
            <Text size="m"><strong>Categories</strong></Text>
